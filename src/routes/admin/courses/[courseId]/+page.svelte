...
<svelte:head>
  <title>Edit Course | CDM Nexus Admins</title>
  <meta
    name="description"
    content="Course editing page for CDM Nexus Administrators."
  />
</svelte:head>
  ...
  >
    <section class="flex w-full flex-col justify-start gap-4 p-2 pb-4">
      <section class="pb-2">
        ...
        <form class="flex flex-col" method="post" action="?/updateCourse">
          ...
          <fieldset class="flex flex-row justify-end gap-4">
            <p class="font-nunito inline-block py-2 pr-2 text-red-600">
              {#if form?.updateCourseMessage}
                {form?.updateCourseMessage ?? ''}
              {/if}
            </p>
              ...
              {#each data.lessons as lesson (lesson.id)}
                {#if lesson.term === 'midterm'}
                  <li>
                    <Button
                      variant="outline"
                      href={`/admin/courses/${data.course.slug}/${lesson.slug}`}
                      class={{
                        button: 'w-full',
